...
void Main(){
	...
	while (System::Update()) {
		...
		CodingProcess::IFFT ifft = CodingProcess::IFFT(fft.get_FFT(), sr);
		std::vector<std::complex<double>> ifft_f = ifft.get_IFFT();

		CodingProcess::FFT fft2 = CodingProcess::FFT(ifft_f, sr);
